...
class FunctionCalculator(QWidget):
    ...
    def initUI(self):
        ...
        self.first_radio = QRadioButton('y = 4^x - 10 * 2^(x - 1) - 24')
        self.second_radio = QRadioButton('y = 1 - 1 / (1 - sin^(-1)(2x + 3π/2))')
        function_layout.addWidget(self.function_label)
        function_layout.addWidget(self.first_radio)
        function_layout.addWidget(self.second_radio)
    ...
    def calculate(self):
        if self.first_radio.isChecked():
            func = lambda x: 4**x - 10*2**(x-1) - 24
        elif self.second_radio.isChecked():
            func = lambda x: 1 - 1 / (1 - np.arcsin(2*x + 3*np.pi/2))
        else:
            return
